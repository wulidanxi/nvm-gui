...
### Changed
- 优化 Dashboard 入场、统计刷新与 Node.js 操作反馈动效，统一管理动画生命周期并兼容系统减少动态效果偏好。
- 应用升级至预览版 `0.0.19-alpha.1`；Windows 资源版本映射为 `0.0.19.1`。

### Fixed
- 修复 macOS/Linux 本地 Node.js 版本全部显示为使用中的问题，正确区分当前版本标记和已安装标记，并过滤别名与颜色控制字符。
- 修复 macOS/Linux 更新检测将 `0.0.8b` 等历史版本误判为高于 `0.0.18` 的问题，使用 SemVer 比较并跳过无效版本。

## [0.0.18] - 2026-07-14

### Added
- Dashboard 新增近 7 天本地使用统计，展示操作总数、成功率、平均耗时、Node 切换次数以及安装/卸载次数。
- 每日操作趋势增加成功/失败堆叠柱、动态纵轴、水平参考线和手动刷新入口，统计数据仅来自本机保留的命令日志。

### Changed
- Dashboard 运行时区域根据 NVM 缺失或 Node 未激活状态提供对应处理入口，快捷操作可直接定位到目标设置分区。
- 统计刷新期间保留已有数据；无操作日期不再显示空白柱体，深浅主题统一使用明确的成功与失败状态色。
- 应用由 `0.0.18-beta.2` 升级为稳定版 `0.0.18`；Windows 资源版本映射为 `0.0.18.0`。

### Fixed
- 修复 Dashboard 统计趋势因成功、失败主题变量缺失而无法显示柱体的问题。
- 修复统计数据仅在 Dashboard 首次挂载时读取、用户无法主动更新的问题。

## [0.0.18-beta.2] - 2026-07-13

### Added
- 仪表盘、本地版本、可安装版本、命令日志和设置页统一增加路由切换动效，并为设置页内部功能分区增加横向衔接动画。
- 深浅色主题切换改为从开关点击位置向外传递的柔和水波效果，波前包含轻量回波层次。

### Changed
- 页面切换统一使用仅包含透明度和位移的合成动画，并继续兼容系统“减少动态效果”偏好。
- 主题水波过渡期间临时关闭顶栏和侧栏背景模糊，降低全窗口快照与蒙版合成带来的掉帧。
- 应用版本升级至 `0.0.18-beta.2`；Windows 资源版本映射为 `0.0.18.2`。

### Fixed
- 修复 Node.js 发行数据命中有效缓存时，操作日志仍将本地缓存读取标记为远程 `GET` 请求的问题。

## [0.0.18-beta.1] - 2026-07-13

### Added
- 本地版本、可安装版本和命令日志表格新增每页条数选择器，统一支持每页 `8 / 16 / 24 / 32` 条。
- 可安装版本按主版本展示首次发布日期、最后更新日期以及 `Current / LTS / EOL` 生命周期状态。

### Changed
- 主窗口默认尺寸调整为 `1800 × 1080`，版本表格筛选、数据区和右下分页统一为命令日志页的面板布局。
- 表格数据超过可视高度时改为表格内部滚动，页面标题、统计信息、筛选区和分页保持固定。
- “仅看 LTS”和 LTS 统计仅包含当前仍受支持的 LTS 版本，不再包含已经 EOL 的历史 LTS。
- 应用版本升级至 `0.0.18-beta.1`；Windows 资源版本映射为 `0.0.18.1`。

### Fixed
- 修复发行数据缓存尚未过期时，点击刷新仍通过 `forceRefresh` 绕过缓存并请求远程数据的问题。
- 修复分页条数增大后整个页面滚动、表格自身没有纵向滚动条的问题。

## [0.0.18-alpha.5] - 2026-07-13

### Added
- Node.js 发行数据默认缓存 24 小时，可在高级设置中调整为 `0–168` 小时，并支持手动强制刷新和过期缓存回退。
- 命令日志新增“发行请求”类别、类别筛选以及网络/缓存获取结果记录。

### Changed
- 命令日志存储由整体 JSON 重写改为 JSONL 增量写入，保留 500 条上限、旧日志迁移、损坏行容错和 JSON 导出兼容。
- 应用版本升级至预览版 `0.0.18-alpha.5`；Windows 资源版本映射为 `0.0.18.5`。

### Fixed
- 修复高级设置中缓存时长输入框和说明文字挤压、两行设置控件未对齐的问题。

## [0.0.18-alpha.4] - 2026-07-12

### Fixed
- 固定 Windows NSIS 安装包文件名，确保 GitHub Release 资产名称与 `latest.yml` 下载路径一致，修复应用内下载返回 404。
- 下载失败后保留目标版本和更新内容，并恢复下载按钮的重试能力。

### Changed
- 应用版本升级至预览版 `0.0.18-alpha.4`；Windows 资源版本映射为 `0.0.18.4`。

## [0.0.18-alpha.3] - 2026-07-12

### Fixed
- 修复切换 Node.js 版本后，左下角当前版本和 Dashboard 仍显示旧版本的问题。
- Node.js 版本切换成功后统一刷新共享运行时状态，确保所有入口和展示区域同步更新。

### Changed
- 应用版本升级至预览版 `0.0.18-alpha.3`；Windows 资源版本映射为 `0.0.18.3`。

## [0.0.18-alpha.2] - 2026-07-12

### Changed
- 检测到新版本后主动弹出更新窗口，展示当前版本、新版本号和更新内容。
- 下载更新期间持续显示实时进度，避免只能通过顶部按钮文字判断下载状态。
- Windows 更新包下载完成后自动重启应用并启动安装程序；非 Windows 平台继续跳转发布页手动下载。
- 收敛更新弹窗的尺寸和信息层级，并将 GitHub Release HTML 安全转换为纯文本列表，避免直接显示标签结构。
- 预览版编号统一采用标准 SemVer 格式，发布校验支持 `alpha.N`、`beta.N` 和 `rc.N`。
- 应用版本升级至预览版 `0.0.18-alpha.2`；Windows 资源版本映射为 `0.0.18.2`。
